...
struct ChessModelBinpack : Model {

    /**
     * @brief Base class for Chess models with training setup.
     * Uses Stockfish's binpack data format.
     * Override `setup_inputs_and_outputs` to define inputs.
     * `start lambda`: CP-WDL ratio at the start
     * `end lambda`: CP-WDL ratio interpolated by the end of the training
     */
    ...
    ChessModelBinpack(float start_lambda_ = 0.7, float end_lambda_ = 0.7)
        : start_lambda(start_lambda_)
        , end_lambda(end_lambda_) {}
    ...
    virtual void setup_inputs_and_outputs(binpackloader::DataEntry& positions) = 0;
    ...
    void distribution(binpackloader::BinpackLoader& loader, int batches = 32) {
