...
__global__ void lrelu_bp_kernel(
...
__global__ void lrelu_bp_kernel_fast(
    const float* __restrict__ A,
          float* __restrict__ A_grd,
    const float* __restrict__ B,
    const float* __restrict__ B_grd,
...
inline void lrelu_bp(const data::DenseMatrix<float> &A,
    ...
    if constexpr (data::is_gpu(DEV)) {
        ...
        if (A.ld != A.m || B.ld != B.m) {
            ...
            lrelu_bp_kernel<<<grid, block>>>(A.first<DEV>(),
                                             A_grd.first<DEV>(),
                                             B.first<DEV>(),
                                             B_grd.first<DEV>(),
                                             A.m,
