...
#include "affine_sparse_shared.h"

// clang-format off
__global__ void operations::affine_sparse_shared_kernel(
    ...
    for(int j = 0; j < 2; j++){
        ...
        int count = inp1_col_indices[offset];
        ...
        for (int i = offset + 1; i < offset + 1 + count; i++) {
            ...
            auto wgt = mat[MATRIX_INDEX(ldw, row, b_row)];

            sum += wgt;
        }
        res[MATRIX_INDEX(ldo, row + m * j, col)] = sum;
    }
